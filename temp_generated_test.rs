...
fn process_node_generated_incremental(
    ...
    if node.cached_node_intrinsic.is_none() || node.is_dirty {
        ...
        if node_matches_selector_generated(node, &SimpleSelector::Class("item".to_string())) {
            intrinsic_matches.set_bit(2); // match_Class("item")
        }

        // Instruction 4: CheckAndSetBit { selector: Id("specific"), bit_pos: 4 }
        if node_matches_selector_generated(node, &SimpleSelector::Id("specific".to_string())) {
            intrinsic_matches.set_bit(4); // match_Id("specific")
        }
    ...
    if parent_state.is_bit_set(1) && node_matches_selector_generated(node, &SimpleSelector::Class("item".to_string())) {
